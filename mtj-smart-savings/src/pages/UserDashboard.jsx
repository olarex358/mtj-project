...
export default function UserDashboard() {
  ...
  if (loading) return <Layout><div style={{padding: '40px', textAlign: 'center', color: '#666'}}>Loading your dashboard...</div></Layout>;
      ...
      <div style={{ background: 'linear-gradient(135deg, #00B875 0%, #008F5D 100%)', padding: '24px 20px 40px', color: 'white', borderBottomLeftRadius: '24px', borderBottomRightRadius: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <button 
            ...
            style={{ flex: 1, padding: '14px', background: 'white', color: '#00B875', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
          >
            💵 Deposit
          </button>
          <button 
      ...
      <div style={{ padding: '0 16px', marginTop: '-20px' }}>
        ...
        <div style={{ background: 'white', padding: '16px', borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div>
            ...
            <h3 style={{ margin: '4px 0 0 0', color: '#111827', fontSize: '18px' }}>
              {tier.icon} {profile?.trust_score || 50}/100 
              <span style={{ fontSize: '13px', color: tier.color, marginLeft: '8px', fontWeight: '600' }}>{tier.name}</span>
            </h3>
          </div>
        ...
        {profile?.card_status === 'pending' && (
          <div style={{ background: '#FFFBEB', borderLeft: '4px solid #F59E0B', padding: '14px', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
            ...
            <div>
              <strong style={{ color: '#92400E', display: 'block', fontSize: '13px' }}>Your MTJ Card is Being Prepared</strong>
              <small style={{ color: '#A16207', fontSize: '12px' }}>You can still save using your Digital Card!</small>
            </div>
          </div>
        )}
        ...
        <div style={{ background: 'white', padding: '16px', borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div>
            <strong style={{ color: '#111827', fontSize: '14px' }}>🔒 Lock Daily Savings</strong>
            <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: '#6B7280' }}>Prevent impulsive withdrawals.</p>
          </div>
