...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    fetchPendingCount();
    const channel = supabase.channel('payouts-channel')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'agent_payouts' }, fetchPendingCount)
      .subscribe();
    ...
    <Layout>
      ...
      <div style={{ background: 'linear-gradient(135deg, #00B875 0%, #008F5D 100%)', padding: '24px 20px', color: 'white', borderBottomLeftRadius: '24px', borderBottomRightRadius: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '22px', fontWeight: '700' }}>MTJ Admin Console</h1>
            <p style={{ margin: '4px 0 0 0', fontSize: '13px', opacity: 0.9 }}>Logged in as: {roleDisplay} ADMIN</p>
          </div>
          <div style={{ background: 'rgba(255,255,255,0.2)', padding: '8px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: '600' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', padding: '16px 20px', overflowX: 'auto', background: 'white', borderBottom: '1px solid #f0f0f0' }}>
        {visibleTabs.map(tab => (
          <button
            ...
            style={{
              background: activeTab === tab.id ? '#00B875' : '#F4F6F8',
          ...
          >
            {tab.label}
            {tab.badge > 0 && (
              <span style={{ position: 'absolute', top: '-6px', right: '-6px', background: '#EF4444', color: 'white', fontSize: '10px', fontWeight: 'bold', padding: '2px 6px', borderRadius: '10px', border: '2px solid white' }}>
                {tab.badge}
              </span>
            )}
          </button>
      ...
      <div style={{ padding: '20px', background: '#F4F6F8', minHeight: 'calc(100vh - 200px)' }}>
        ...
        {activeTab === 'settings' && (
          <div style={{ background: 'white', padding: '24px', borderRadius: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
            <h2 style={{ margin: '0 0 12px 0', color: '#111827', fontSize: '18px' }}>⚙️ System Settings</h2>
            <p style={{ color: '#EF4444', fontWeight: '600' }}>🔒 Restricted to Super Admin only.</p>
          </div>
        )}
      </div>
    </Layout>
